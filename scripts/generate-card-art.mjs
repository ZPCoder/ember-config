...
const cardsToGenerate = CARD_CATALOG.filter((card) => card.id.includes("-season-") && (!existingFiles.includes(`${card.id}.webp`) || process.env.FORCE_CARD_ART === "1"));
for (let position = 0; position < cardsToGenerate.length; position += 1) {
  ...
  const candidates = existingByPrefix.get(theme.slug) ?? [];
